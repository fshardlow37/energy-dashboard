export function createTooltipHandler(containerEl) {
  ...
  return function externalTooltipHandler(context) {
    ...
    // Display order matches stack order (top to bottom)
    const DISPLAY_ORDER = ['Wind', 'Solar', 'Gas', 'Imports', 'Nuclear', 'Biomass', 'Coal', 'Hydro', 'Other'];
    const itemMap = {};
    for (const dp of dataPoints) {
      itemMap[dp.dataset.label] = {
        label: dp.dataset.label,
        value: dp.raw.y,
        color: dp.dataset.backgroundColor.replace(/[0-9a-f]{2}$/i, '')
      };
    }
  ...
  return function externalTooltipHandler(context) {
    ...
    tooltipEl.innerHTML = `
      ...
      <div class="row">
        ...
        <span class="value">${demandGW} GW</span>
      </div>`;
    tooltipEl.classList.remove('hidden');
